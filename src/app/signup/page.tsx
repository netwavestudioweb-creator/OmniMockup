...
import { Layers, Mail, Lock, ArrowRight, AlertCircle, Loader2, CheckCircle2 } from 'lucide-react';
...
function SignupForm() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si l'auto-confirmation est activée ou que la session existe
      if (data.session) {
    ...
    return (
      <div className="min-h-screen bg-sand-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md px-4">
          <div className="bg-white py-10 px-6 sm:px-10 shadow-xl shadow-stone-900/5 rounded-3xl border border-sand-200 text-center space-y-4">
            <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-2xl flex items-center justify-center mx-auto">
              <CheckCircle2 className="w-6 h-6" />
            </div>
            <h2 className="text-xl font-bold text-stone-900">Vérifiez votre boîte de réception</h2>
            <p className="text-xs sm:text-sm text-stone-600 leading-relaxed">
              Un lien de confirmation a été envoyé à <strong className="text-stone-900">{email}</strong>. Cliquez dessus pour activer votre compte et débloquer vos analyses IA.
            </p>
            <div className="pt-4">
              <Link
                href="/login"
    ...
    <div className="min-h-screen bg-sand-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p className="mt-2 text-xs sm:text-sm text-stone-500">
          3 analyses IA avec justification offertes chaque mois
        </p>
      </div>
        ...
        <div className="bg-white py-8 px-6 sm:px-10 shadow-xl shadow-stone-900/5 rounded-3xl border border-sand-200">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-white text-stone-400 font-medium">ou avec un email</span>
            </div>
          </div>
